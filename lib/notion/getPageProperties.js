...
export default async function getPageProperties(id, block, schema, authToken, tagOptions) {
  console.log(`当前id:${id} block:${block} schema:${schema} authToken:${authToken} tagOptions:${tagOptions}`)
  const rawProperties = Object.entries(block?.[id]?.value?.properties || [])
  const excludeProperties = ['date', 'select', 'multi_select', 'person']
  const value = block[id]?.value
  const properties = {}
  console.log(`当前id:${id} 1`)
  for (let i = 0; i < rawProperties.length; i++) {
    const [key, val] = rawProperties[i]
    properties.id = id
    if (schema[key]?.type && !excludeProperties.includes(schema[key].type)) {
      console.log(`当前id:${id} 2`)
      properties[schema[key].name] = getTextContent(val)
    } else {
      console.log(`当前id:${id} 3`)
      console.log(`当前id:${schema[key]?.type} 4`)
      switch (schema[key]?.type) {
        case 'date': {
          const dateProperty = getDateValue(val)
          console.log(`当前id:${dateProperty}  ${val} 5`)
          if (dateProperty !== null && dateProperty !== undefined) {
            delete dateProperty.type
            properties[schema[key].name] = dateProperty
            console.log(`当前id:${schema[key]}  ${properties[schema[key].name]} 6`)
          }
          break
        }
        case 'select':
